...
- **Couverture des exercices 2 à 5** : les templates ne publient de série annuelle (`projection`) que pour le CA et le résultat net, sur 3 ans. Tant que FIN-001 n’a pas étendu l’horizon et les séries, le suivi au-delà de l’exercice 1 est comparable pour ces seules lignes — le reste est honnêtement marqué `EXERCICE_ABSENT_DU_PLAN`. Aucun changement côté réalisé ne sera nécessaire : la résolution de base annuelle prendra les nouvelles lignes `*_annuel_4/5` dès qu’elles existeront.
